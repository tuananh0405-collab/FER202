...
function Home({ projects, departments }) {
  ...
  const [selectedSingleDepartment, setSelectedSingleDepartment] =
    useState("All");

  const filterProjects = () => {
    if (selectedSingleDepartment !== "All") {
      return projects.filter(
        (project) => project.department.toString() === selectedSingleDepartment
      );
    }
    if (
  ...
  const handleMultiDepartmentChange = (event) => {
    ...
    if (value === "All") {
      ...
      if (selectedDepartments.includes(value)) {
        ...
        setSelectedDepartments([
          ...selectedDepartments.filter((id) => id !== "All"),
          value,
        ]);
      }
    }
  ...
  return (
    <Container>
      ...
      <Row>
        ...
        <Col md={9}>
          <Link to={"/projects/add"} className="">
            <Button variant="success">Create new Project</Button>
          </Link>
                    ...
                    <Link to={`/departments/${project.department}/employees`}>
                      {
                        departments.find(
                          (dept) =>
                            dept.id.toString() === project.department.toString()
                        )?.name
                      }
                    </Link>
                  </td>
